...
from ..utils.device import get_default_device, safe_autocast_decorator
...
class InferenceCore:
    """Core inference logic for MatAnyone2."""
        ...
        *,
        ...
        if self.flip_aug:
            self.last_mask = torch.cat(
                [
                    self.last_mask,
                    torch.flip(self.last_mask, dims=[-1]),
                ],
                dim=0,
            )
    ...
    def output_prob_to_mask(
        ...
        if matting:
            return output_prob[1:].squeeze(0)
